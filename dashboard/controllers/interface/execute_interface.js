...
exports = module.exports = (app, db, errors) => {
    ...
    let executeCommand = (server, modelInstance, filter, data, access_token) => {
        ...
        return new Sequelize.Promise((resolve, reject) => {
            ...
            _.each(modelInstance.api.split('/'),(api)=>{
                "use strict";
                if(api.search(':')==0){
                    api = data[api.substring(1)];
                }
                apis.push(api);
            });
            modelInstance.api = apis.join('/');
            ...
            switch (modelInstance.verb.toUpperCase()) {
                ...
                case "POST":
                    htp = superagent.post(uri)
                        .type("form")
                        .send(!modelInstance.needDatas ? null : data)
                        .timeout(5000);
                    modelInstance.needParams && htp.query(params);
                    break;
                case "PUT":
                    htp = superagent.put(uri)
                        .type("form")
                        .send(!modelInstance.needDatas ? null : data)
                        .timeout(5000);
                    modelInstance.needParams && htp.query(params);
